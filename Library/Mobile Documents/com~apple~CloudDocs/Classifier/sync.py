...
def _update_github_secret(secret_name: str, secret_value: str) -> None:
    """Update a GitHub Actions secret via the REST API using GH_PAT_SECRETS."""
    import base64
    import subprocess
    ...
    if not repo or not pat:
        print(f"[WARN] Skipping GitHub secret update for {secret_name}: "
              "GH_PAT_SECRETS or GITHUB_REPOSITORY not set")
        return

    try:
        # Use gh CLI — it handles Sodium encryption automatically
